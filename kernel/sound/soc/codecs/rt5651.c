...
#define rt5651_CODEC_SET_SPK	1
#define rt5651_CODEC_SET_HP	2
...
static struct rt5651_priv *rt5651_private;
static int rt5651_set_gpio(int gpio, bool level)
{
	struct rt5651_priv *rt5651 = rt5651_private;

	if (!rt5651) {
		return 0;
	}

	if ((gpio & rt5651_CODEC_SET_SPK) && rt5651
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	if (gpio_get_value(rt5651->hp_det_gpio))
		rt5651->hp_inserted = 0;
	else
		rt5651->hp_inserted = 1;

	if (rt5651->muted == 0) {
		if (rt5651->hp_det_level != rt5651->hp_inserted)
			rt5651_set_gpio(rt5651_CODEC_SET_SPK, !rt5651->spk_gpio_level);
		else
			rt5651_set_gpio(rt5651_CODEC_SET_SPK, rt5651->spk_gpio_level);
	}
	return IRQ_HANDLED;
...
static int rt5651_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		...
		//snd_soc_write(codec, rt5651_DACCONTROL3, 0x06);
	} else {
		//snd_soc_write(codec, rt5651_DACCONTROL3, 0x02);
		//snd_soc_write(codec, 0x30, rt5651_DEF_VOL);
		//snd_soc_write(codec, 0x31, rt5651_DEF_VOL);
		msleep(50);
		    ...
		    const struct i2c_device_id *id)
{
	...
	rt5651_private = rt5651;
	rt5651->spk_ctl_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "spk-con-gpio", 0, &flags);
	if (rt5651->spk_ctl_gpio < 0) {
		dev_info(&i2c->dev, "Can not read property spk_ctl_gpio\n");
		rt5651->spk_ctl_gpio = INVALID_GPIO;
	} else {
		rt5651->spk_gpio_level = (flags & OF_GPIO_ACTIVE_LOW) ? 0 : 1;
		ret = devm_gpio_request_one(&i2c->dev, rt5651->spk_ctl_gpio, GPIOF_DIR_OUT, "spk-con-gpio");
		if (ret != 0) {
	...
	rt5651->hp_det_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "hp-det-gpio", 0, &flags);
	if (rt5651->hp_det_gpio < 0) {
		dev_info(&i2c->dev, "Can not read property hp_det_gpio\n");
		rt5651->hp_det_gpio = INVALID_GPIO;
	} else {
		rt5651->hp_det_level = (flags & OF_GPIO_ACTIVE_LOW) ? 0 : 1;
		ret = devm_gpio_request_one(&i2c->dev, rt5651->hp_det_gpio, GPIOF_IN, "hp-det-gpio");
		if (ret != 0) {
		...
		if (hp_irq) {
			ret = devm_request_threaded_irq(&i2c->dev, hp_irq, NULL, hp_det_irq_handler,
					IRQ_TYPE_EDGE_BOTH | IRQF_ONESHOT,"rt5651", NULL);
			if (ret < 0) {
...
void rt5651_i2c_shutdown(struct i2c_client *client)
{
	struct rt5651_priv *rt5651 = rt5651_private;
